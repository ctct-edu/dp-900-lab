...
1. 次の詳細を入力して、**[レビュー + 作成]** を選択します。
    ...
    - **場所**:  **(US) West US 2** (リージョンが選択できない場合はページを再読み込み)
    - **容量モード**: プロビジョニングされたスループット
    - **Apply Free-Tier Discount (Free レベル割引の適用)**: 使用できる場合は [適用] を選択
    - **合計アカウント スループットを制限する**: **オフ（チェックを外す）**
1. 構成を確認したら、**[レビュー + 作成]**、**[作成]** の順にクリックします。
...
1. **[データエクスプローラー]** ページで、**[クイックスタートの起動(Launch quick start)]** を選びます。
...
1. [データエクスプローター] ページで **SampleDB** データベースと **SampleContainer** コンテナーを展開し、**[Items]** を選んでコンテナー内の項目の一覧を表示します。 項目は製品データを表し、それぞれに一意の ID と他のプロパティがあります。
1. 一覧内の項目を選ぶと、項目データの JSON 表現が表示されます。
1. ページの上部にある **[新しい項目(New Item)]** を選び、新しい空白の項目を作成します。
1. 新しい項目の JSON を以下のように変更し、**[保存(Save)]** を選びます。
...
1. SQL クエリ エディターで既定のクエリ (`SELECT * FROM c`) を確認し、**[クエリの実行(Execute Query)]** ボタンを使って実行します。
...
1. **[クエリの実行(Execute Query)]** ボタンを使って、修正したクエリを実行し、結果を確認します。これには、**name** フィールドに "Helmet" というテキストが含まれる項目の JSON エンティティが含まれます。
